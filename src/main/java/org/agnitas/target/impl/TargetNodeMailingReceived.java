...
public class TargetNodeMailingReceived extends TargetNode implements Serializable {
	...
	public String generateEmbeddedSQL() {
		...
		if( AgnUtils.isProjectEMM()) {
			buffer.append( "mailtrack_");
			buffer.append( companyId);
			buffer.append( "_tbl");
		} else {
			buffer.append( "mailtrack_tbl");
		}

		buffer.append( " mt, maildrop_status_tbl mds WHERE mds.company_id=");
		buffer.append( companyId);
		buffer.append( " AND mds.mailing_id=");
		buffer.append( mailingId);
		buffer.append( " AND mds.status_id=mt.status_id AND mt.customer_id=cust.customer_id)");
