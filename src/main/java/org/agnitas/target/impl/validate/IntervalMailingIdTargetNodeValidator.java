...
import org.agnitas.target.TargetError.ErrorKey;
 ...
 *
 */
public class IntervalMailingIdTargetNodeValidator implements TargetNodeValidator {
	...
	public Collection<TargetError> validate(TargetNode node, @VelocityCheck int companyId) {
		try {
			...
			if( mailing == null || mailing.getId() != mailingId)
				return reportError( TargetError.ErrorKey.INVALID_MAILING);
			if( mailing.getMailingType() != Mailing.TYPE_INTERVAL)
				return reportError( TargetError.ErrorKey.NOT_AN_INTERVAL_MAILING);

			return null;
		} catch( Exception e) {
			return reportError( TargetError.ErrorKey.CANNOT_VALIDATE);
		}

	}

	/**
	 * Report specified errors.
	 *
	 * @param errorKey key of error to report
	 *
	 * @return error result for invalid mailing ID
	 */
	private Collection<TargetError> reportError( ErrorKey errorKey) {
		Collection<TargetError> errors = new Vector<TargetError>();
		errors.add( new TargetError( errorKey));
