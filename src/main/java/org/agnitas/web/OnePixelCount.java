...
/**
 * The servlet is used for registration of opening emails have been sent to
 * customers and loading one pixel gif image that is automatically adding into
 * content of each OpenEMM mailing. Each time the customer opens the email, the
 * request is sent to the server, the data about the mailing and customer are
 * logged in database and the server returns http response that contains one
 * pixel gif image.
 */
public class OnePixelCount extends HttpServlet {
	...
	protected byte[] onePixelGif = { 71, 73, 70, 56, 57, 97, 1, 0, 1, 0, -128, -1, 0, -64, -64, -64, 0, 0, 0, 33, -7, 4, 1, 0, 0, 0, 0, 44, 0, 0, 0, 0, 1, 0, 1, 0, 0, 2, 2, 68, 1,
			0, 59 };
	...
	public void service(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
			...
			try {
				...
				if (uid != null && uid.getCompanyID() > 0) {
					...
					if (company == null) {
						 ...
						 * logger.warn("uid invalid: " + param); return; }
						 */
		...
		accessLogging(req, uid);
	}

	/**
	 * Calls writePixelLogToDB method and logs to console in case of its
	...
	/**
	 * Stores opened email data (company id, mailing id, customer id, client IP
	 * address) in database.
	 * 
	 * @param req
	 *            HTTP request, is used fot getting the IP address of the client
	...
	/**
	 * Gets the id of action to be exequted on opening the mailing; if the
	 * action id > 0, executes the action with parameters from the request.
	 * 
	 * @param uid
	 *            ExtensibleUID object, contains parsed data from the "uid"
	 *            request parameter
	...
	protected void accessLogging(HttpServletRequest req, ExtensibleUID uid) {
		// no logging
	}
}
