...
public class UserFormExecuteAction extends StrutsActionBase {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
		...
		try {
			...
			processUID(req, params, aForm.getAgnUseSession());
			...
			if (params.get("_error") == null) {
				evaluateFormEndAction(aForm, params);
	 ...
	 *            html content to be sent in response (could be changed inside the method).
	...
	/**
	 * Execute the requested form. Reads the form defined by aForm.getAgnFN() and aForm.getAgnCI() from the database and executes it.
	 * 
	 * @param aForm
	 *            form info.
	 * @param params
	 *            a map containing the form values.
	 * @param req
	 *            the ServletRequest, used to get the ApplicationContext.
	 * @param errors
	 *            used to sotre error descriptions.
	...
	protected String executeForm(UserFormExecuteForm aForm, Map<String, Object> params, HttpServletRequest req, ActionMessages errors) throws Exception {
		...
		if (aUserForm != null) {
			result = aUserForm.evaluateForm(getWebApplicationContext(), params);
		} else {
	...
	protected boolean evaluateFormEndAction(UserFormExecuteForm aForm, Map<String, Object> params) throws Exception {
		...
		return aUserForm.evaluateEndAction(getWebApplicationContext(), params);
	}
	...
	/**
	 * Use a tag to get a UID. Retrieves a UID according to a given tag.
	 * 
	 * @param tag
	 *            a string defining the uid.
	 * @return the resulting UID.
	 */
	public ExtensibleUID decodeTagString(String tag) {
		ExtensibleUID uid = null;

		ExtensibleUIDService uidService = (ExtensibleUIDService) getWebApplicationContext().getBean(ExtensibleUIDConstants.SERVICE_BEAN_NAME);
