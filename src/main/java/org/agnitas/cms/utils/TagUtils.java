...
public class TagUtils {
	...
	public static final String[] TAG_NAMES = { "agnTEXT", "agnLABEL", "agnIMAGE", "agnLINK" };
	...
	public static String generateContentModuleContent(int contentModuleId, boolean migrateImageTags, ApplicationContext aContext) {
		ContentModuleManager contentModuleManager = (ContentModuleManager) aContext.getBean("ContentModuleManager");
		ContentModule contentModule = contentModuleManager.getContentModule(contentModuleId);
		List<CmsTag> placeholders = contentModuleManager.getContentModuleContents(contentModuleId);
		...
		for (String tag : tags) {
			...
			if (getTagType(tag) == TAG_IMAGE && migrateImageTags) {
				...
				replacement = "[" + TAG_NAMES[TAG_IMAGE] + " name=\"" + tagName + "_" + contentModule.getId() + "\"]";
			}
		...
		if (content != null) {
			...
			while (tagStart != -1) {
				...
				if ((nextTagStart < tagEnd && nextTagStart != -1) || tagEnd == -1) {
					content = content.substring(0, tagStart) + "TAG_WAS_HERE" + content.substring(tagStart + 4);
				} else {
					String tag = content.substring(tagStart, tagEnd + 1);
					if (isTagValid(tag)) {
						tags.add(tag);
					}
					content = content.substring(0, tagStart) + "TAG_WAS_HERE" + content.substring(tagEnd + 1);
				}
	...
	public static MediaFile getMediaFileForTag(List<MediaFile> mediaFiles, CmsTag tag, int contentModuleId) {
		for (MediaFile mediaFile : mediaFiles) {
			if (tag.getName().equals(mediaFile.getName()) && mediaFile.getContentModuleId() == contentModuleId) {
				return mediaFile;
			}
		}
		return null;
	...
	public static CmsImageTag createImageTag(String tagName, int tagType, String newValue, String imagePath) {
