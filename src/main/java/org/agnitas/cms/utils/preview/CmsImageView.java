 ...
 * @since 1.4
 */
public class CmsImageView extends View {
	/**
	 ...
	 */
	public CmsImageView(Element elem) {
		super(elem);
	...
	public void setSize(float width, float height) {
		...
		if(getImage() == null) {
			...
			if(view != null) {
				view.setSize(Math.max(0f,
						width - (float) (DEFAULT_WIDTH + leftInset + rightInset)),
	...
	private class ImageHandler implements ImageObserver {
		...
		public boolean imageUpdate(Image img, int flags, int x, int y,
			...
			if((flags & (ABORT | ERROR)) != 0) {
				repaint(0);
				synchronized(CmsImageView.this) {
					if(image == img) {
