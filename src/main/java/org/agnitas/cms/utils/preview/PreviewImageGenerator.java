...
import org.agnitas.cms.utils.MediaFileUtils;
...
/**
 * This class generate image from browser`s page. Stores it image in database.
 */
public class PreviewImageGenerator {
    ...
    protected boolean imagesLoaded = false;
	...
	public PreviewImageGenerator(ApplicationContext applicationContext, HttpSession session, final int previewMaxWidth, final int previewMaxHeight) {
		...
		final LinkedBlockingQueue<Runnable> runnables = new LinkedBlockingQueue<Runnable>();
		threadPool = new ThreadPoolExecutor(nThreads, maxPoolThreads, keepAliveTime, TimeUnit.SECONDS, runnables);
		this.previewMaxWidth = previewMaxWidth;
		this.previewMaxHeight = previewMaxHeight;
	}

	/**
	 * Generate preview image for one of cms`s element id, element for wich generates preview must be non zero value and other two must be equals to zero.
	 *
	 * @param cmTemplateId
	 *            id of cms`s template
	 * @param cmId
	 *            id of content module
	 * @param cmtId
	 *            id of content module type
	 */
	...
	private void storeMailingPreview(Integer mailingId, Integer companyId, boolean bulkGenerate) {
		...
		if (!testAdnAdminRecipients.isEmpty()) {
			...
			String previewUrl = "/mailingsend.do;jsessionid=" + session.getId() + "?action=16&mailingID=" + mailingId + "&previewFormat=1&previewCustomerID="
					+ (Integer) testAdnAdminRecipients.keySet().toArray()[0] + "&previewDay=0&previewMonth=0&previewYear=0";

			String systemUrl = AgnUtils.getEMMProperty("system.url");
			final List<MailingComponent> mailingComponents = componentDao.getMailingComponents(mailingId, companyId, MailingComponent.TYPE_THUMBMAIL_IMAGE);
			if (bulkGenerate) {
    ...
    protected byte[] generatePreview(String url, boolean isMailingPreview) {
		if (logger.isInfoEnabled())
		...
		CmsEditorKit editorKit = new CmsEditorKit() {
			private static final long serialVersionUID = 8310487707074752606L;
	...
	private void onPageLoaded() {
		...
		editor.addNotify();
		editor.validate();
		BufferedImage dummyImage = new BufferedImage(20, 20, BufferedImage.TYPE_INT_RGB);
	...
	private byte[] renderPreview(boolean isMailingPreview) {
		if (imagesLoaded && pageLoaded && !rendered) {
			...
			if (minimumSize.width > currentSize.width || minimumSize.height > currentSize.height) {
			...
			// paint page on image
			BufferedImage originalImage = new BufferedImage(currentSize.width, currentSize.height, BufferedImage.TYPE_INT_RGB);
	...
	private void storePreview(byte[] imageData) {
		MediaFile mediaFile = new MediaFile(cmTemplateId, 1, imageData, cmId, cmtId, 0, MediaFileUtils.PREVIEW_TYPE, "image/png", "preview");
		if (cmId != 0) {
	...
	private String generatePreviewUrl(int cmTemplateId, int cmId, int cmtId) {
		...
		if (cmId > 0) {
			return "/cms_contentmodule.do;jsessionid=" + sessionId + "?action=" + ContentModuleAction.ACTION_PURE_PREVIEW + "&contentModuleId=" + cmId;
		} else if (cmTemplateId > 0) {
			return "/cms_cmtemplate.do;jsessionid=" + sessionId + "?action=" + CMTemplateAction.ACTION_PURE_PREVIEW + "&cmTemplateId=" + cmTemplateId;
		} else if (cmtId > 0) {
			return "/cms_cmt.do;jsessionid=" + sessionId + "?action=" + ContentModuleTypeAction.ACTION_PURE_PREVIEW + "&cmtId=" + cmtId;
		} else {
			return null;
		}
	}
