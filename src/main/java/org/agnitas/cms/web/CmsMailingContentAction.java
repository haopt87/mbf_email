...
public class CmsMailingContentAction extends MailingContentAction {
	private static final transient Logger logger = Logger.getLogger(CmsMailingContentAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		ActionForward actualDestination = super.execute(mapping, form, request, response);
		...
		try {
			...
			case CmsMailingContentAction.ACTION_VIEW_CONTENT: {
				...
				Map<Integer, String> testRecipients = recipientDao.getAdminAndTestRecipientsDescription(aForm.getCompanyID(request), aForm.getMailingID());
				...
				if (template == null) {
					List<Integer> assignedCms = contentModuleManager.getAssignedCMsForMailing(aForm.getMailingID());
			...
			case CmsMailingContentAction.ACTION_SAVE_CMS_DATA: {
				...
				if (cmEditId != null && !cmEditId.isEmpty() && !"0".equals(cmEditId)) {
					destination = new ActionForward(mapping.findForward("edit_cm").getPath() + "&contentModuleId=" + cmEditId + "&mailingId=" + aForm.getMailingID(), true);
				} else if (phForNewCM != null && !phForNewCM.isEmpty()) {
					destination = new ActionForward(mapping.findForward("new_cm").getPath() + "&mailingId=" + aForm.getMailingID() + "&phName=" + phForNewCM
							+ "&createForMailing=true", true);
				} else {
					aForm.setAction(CmsMailingContentAction.ACTION_SAVE_CMS_DATA);
					destination = mapping.findForward("cms_editor_frame");
				}
			...
			case CmsMailingContentAction.ACTION_SHOW_TEXT_VERSION: {
				// Do some thing
				aForm.setTextVersion(cmTemplateManager.getTextVersion(adminId));
			...
			case CmsMailingContentAction.ACTION_SAVE_CMS_TEXT_VERSION: {
				...
				cmTemplateManager.saveTextVersion(adminId, aForm.getTextVersion());
		...
		if (destination != null && "cms_editor_frame".equals(destination.getName())) {
			...
			request.setAttribute("availableCmCount", aForm.getAvailableContentModules().size());
		}
	...
	private boolean isWorldMailingSend(int mailingID, HttpServletRequest request) {
		Mailing aMailing = mailingDao.getMailing(mailingID, AgnUtils.getCompanyID(request));
		return aMailing.isWorldMailingSend();
	}

	private String getParameterValueFromName(HttpServletRequest request, String startStr) {
		@SuppressWarnings("rawtypes")
		Enumeration parameterNames = request.getParameterNames();
		while (parameterNames.hasMoreElements()) {
			String parameterName = (String) parameterNames.nextElement();
			if (parameterName.startsWith(startStr) && parameterName.endsWith(".x")) {
				...
				return parameterName.substring(delimiterIndex + 1, parameterName.length() - 2);
			}
		}
		return null;
	}

	private void saveCmAssignments(CmsMailingContentForm aForm, HttpServletRequest request) {
		List<Integer> oldAssignedCMs = contentModuleManager.getAssignedCMsForMailing(aForm.getMailingID());
		...
		contentModuleManager.addMailingBindingToContentModules(cmsToAssign, aForm.getMailingID());
		contentModuleManager.removeMailingBindingFromContentModules(cmsToDeassign, aForm.getMailingID());
	}

	private void removeContentModule(CmsMailingContentForm aForm, HttpServletRequest request) {
		Integer cmId = Integer.valueOf(getParameterValueFromName(request, "removeCM"));
		aForm.getContentModules().remove(cmId);
		ContentModuleLocation location = CMLocationsUtils.getLocationForCMId(aForm.getContentModuleLocations(), cmId);
		aForm.getContentModuleLocations().remove(location);
		refreshLocations(aForm);
	}

	private ActionMessages addContentModule(CmsMailingContentForm aForm, HttpServletRequest request) {
		...
		String cmContent = TagUtils.generateContentModuleContent(cmId, false, getApplicationContext(request));
	...
	private void refreshLocations(CmsMailingContentForm aForm) {
		List<ContentModuleLocation> validLocations = CMLocationsUtils.createValidLocations(aForm.getContentModuleLocations(), aForm.getContentModules().keySet(),
				aForm.getPlaceholders(), aForm.getCmTemplateId(), aForm.getMailingID());
	...
	private List<ContentModule> getAllContentModulesList(CmsMailingContentForm aForm) {
		...
		for (ContentModule contentModule : aForm.getAllContentModules()) {
			if (!assignedCmIds.contains(contentModule.getId())) {
				if (aForm.getCategoryToShow() == 0 || aForm.getCategoryToShow() > 0 && aForm.getCategoryToShow() == contentModule.getCategoryId()) {
	...
	private void loadAllContentModules(CmsMailingContentForm aForm, HttpServletRequest request) {
		aForm.setAllContentModules(contentModuleManager.getContentModulesForMailing(aForm.getMailingID()));
	}

	private void saveLocations(CmsMailingContentForm aForm, HttpServletRequest request, boolean storeToDB) {
		...
		for (Integer cmId : modules.keySet()) {
			...
			int phOrder = Integer.parseInt(request.getParameter("cm." + cmId + ".order_in_ph"));
		...
		if (storeToDB) {
			contentModuleManager.removeCMLocationsForMailing(aForm.getMailingID());
			contentModuleManager.addCMLocations(aForm.getContentModuleLocations());
		}
	}

	private ActionMessages loadEditor(CmsMailingContentForm aForm, HttpServletRequest request) {
		// load template data
		CMTemplate template = cmTemplateManager.getCMTemplateForMailing(aForm.getMailingID());
	...
	private void loadCMLocations(CmsMailingContentForm aForm, int templateId, HttpServletRequest request) {
		List<ContentModuleLocation> locations = contentModuleManager.getCMLocationsForMailingId(aForm.getMailingID());
		List<ContentModuleLocation> validLocations = CMLocationsUtils.createValidLocations(locations, aForm.getContentModules().keySet(), aForm.getPlaceholders(), templateId,
				aForm.getMailingID());
		aForm.setContentModuleLocations(validLocations);
	}

	private void loadContentModules(CmsMailingContentForm aForm, HttpServletRequest request) {
		List<ContentModule> moduleList = contentModuleManager.getContentModulesForMailing(aForm.getMailingID());
		Map<Integer, String> contentMap = new HashMap<Integer, String>();
		for (ContentModule contentModule : moduleList) {
			String cmContent = TagUtils.generateContentModuleContent(contentModule.getId(), false, getApplicationContext(request));
			cmContent = CmsUtils.appendImageURLsWithSystemUrl(cmContent);
	...
	private ActionMessages FindDynTags(CmsMailingContentForm aForm, HttpServletRequest request, String templateContent) {
		ActionMessages errors = new ActionMessages();
		MailingImpl tmpMailing = new MailingImpl();
		tmpMailing.setCompanyID(AgnUtils.getCompanyID(request));
		tmpMailing.setDynTags(new HashMap<String, DynamicTag>());
		try {
			Vector<String> dynTags = tmpMailing.findDynTagsInTemplates(templateContent, getApplicationContext(request));
			for (Object dynObject : dynTags) {
				...
				if (tagStart > -1 && tagEnd > -1) {
					...
					templateContent = templateContent.substring(0, tagStart) + tagReplacer + templateContent.substring(tagEnd);
				}
			}
			...
			logger.error("Error during dyn tags parsing: " + e, e);
			errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage("error.template.dyntags"));
		}
		return errors;
	}

	private void adjustTagsPositions(Map<String, DynamicTag> dynTags, int shift) {
	...
	private void loadTargetGroups(CmsMailingContentForm aForm, HttpServletRequest request) {
		Map<Integer, CmsTargetGroup> targets = new HashMap<Integer, CmsTargetGroup>();
		Map<Integer, CmsTargetGroup> targetGroups = cmsMailingDao.getTargetGroups(AgnUtils.getCompanyID(request));
		Iterator<Integer> iterator = targetGroups.keySet().iterator();
		CmsTargetGroup target = null;
		while (iterator.hasNext()) {
			Integer key = (Integer) iterator.next();
			target = targetGroups.get(key);
			if (!target.isDeleted()) {
				targets.put(key, target);
			}
		}
		aForm.setTargetGroups(targets);
	}
